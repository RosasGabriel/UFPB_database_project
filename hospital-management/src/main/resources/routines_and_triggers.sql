...
CREATE OR REPLACE PROCEDURE public.sp_registrar_atendimento_completo(
    p_id_paciente INT,
    p_id_residente INT,
    p_id_preceptor INT,
    p_id_unidade INT,
...
DECLARE
    ...
    v_proc RECORD;
BEGIN
    -- 1. Inserir o Atendimento e recuperar o ID gerado
    ...
    -- 2. Ler o array JSONB e inserir cada procedimento na tabela procedimento_realizado
    IF p_procedimentos IS NOT NULL THEN
        FOR v_proc IN SELECT * FROM jsonb_to_recordset(p_procedimentos) AS x(id_procedimento INT, tempo_real INT)
        LOOP
            INSERT INTO procedimento_realizado (id_atendimento, id_procedimento, tempo_real_minutos)
            VALUES (v_id_atendimento, v_proc.id_procedimento, v_proc.tempo_real);
        END LOOP;
    END IF;
END;
$$^;
